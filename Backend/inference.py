...
def parse_args():
    ...
    parser.add_argument("--model-path", type=str, default=os.path.join(script_dir, "model", "last.pt"), help="Path to the model weights")
    parser.add_argument("--image-path", type=str, default=os.path.join(script_dir, "1.jpg"), help="Path to the input image")
...
def main(params):
    ...
    segmentation_image.save(f"{output_dir}/{filename[:-4]}_mask.png")
    ...
    if params.save_overlay:
        ...
        cv2.imwrite(f"{output_dir}/{filename[:-4]}_mask_color.png", colored_class_map)
        cv2.imwrite(f"{output_dir}/{filename[:-4]}_overlay.png", overlayed_image, [int(cv2.IMWRITE_JPEG_QUALITY), 100])
